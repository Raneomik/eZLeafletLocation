<?php
//
// Definition of ezlfltLocationFilter class
//
...
class ezlfltLocationFilter
{
    function ezlfltLocationFilter()
    {
    }
    ...
    function createSqlParts( $params )
    {
         ...
         * {def $users_in_oslo = fetch( 'content', 'tree', hash(
                                      ...
                                      'extended_attribute_filter', hash( 'id', 'ezlfltlLocationFilter', 'params', hash( 'latitude', 59.917, 'longitude', 10.729, 'distance', 0.35 ) )
                                      ) )}
         *
        ...
        $sqlSelect = ",
            ezleafletlocation.latitude AS latitude,
            ezleafletlocation.longitude AS longitude,
            SQRT(POW(ezleafletlocation.latitude - $latitude,2) + POW( ezleafletlocation.longitude - $longitude,2)) AS distance";

        if ( $arccosine )
        {
            $sqlSelect .= ",
                acos( cos(radians( $latitude )) * cos(radians( ezleafletlocation.latitude )) *
                     cos(radians( ezleafletlocation.longitude  -  $longitude )) +
                     sin(radians( $latitude )) * sin( radians( ezleafletlocation.latitude ))) AS arccosine_distance";
        }

        $sqlWhere = "ezleafletlocation.latitude >= $minLatitude AND ezleafletlocation.latitude <= $maxLatitude AND
                     ezleafletlocation.longitude >= $minLongitude AND ezleafletlocation.longitude <= $maxLongitude AND";

        if ( $asCircle === 'pythagorean' )
        {
            $sqlWhere .= "
                     SQRT(POW(ezleafletlocation.latitude - $latitude,2) + POW(ezleafletlocation.longitude - $longitude,2)) <= $distance AND";
        }
        else if ( $asCircle === 'arccosine' )
        {
            $sqlWhere .= "
                     acos( cos(radians( $latitude )) * cos(radians( ezleafletlocation.latitude )) *
                     cos(radians( ezleafletlocation.longitude  -  $longitude )) +
                     sin(radians( $latitude )) * sin( radians( ezleafletlocation.latitude ))) <= $aDistance AND";
        }

        $sqlWhere .= "
                     ezcontentobject_attribute.id = ezleafletlocation.contentobject_attribute_id AND
                     ezcontentobject_attribute.version = ezleafletlocation.contentobject_version AND
        ...
        return array('tables' => ', ezcontentobject_attribute, ezleafletlocation', 'joins' => $sqlWhere, 'columns' => $sqlSelect );
    }
}
?>